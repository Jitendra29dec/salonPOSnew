...
public class BrandListAdapter extends RecyclerView.Adapter<BrandListAdapter.ViewHolder> implements Filterable {
    private Context context;
    private List<BrandData> list;
    private List<BrandData> filterList;
    private ApiCommunicator apiCommunicator;

    public BrandListAdapter(Context context, List<BrandData> list, ApiCommunicator apiCommunicator) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        holder.name.setText(filterList.get(position).getBrandName());
        Glide.with(context)
    ...
    public Filter getFilter() {
        return new Filter() {
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                List<BrandData> tempList = new ArrayList<>();
                if (constraint.toString().equalsIgnoreCase("")) {
                    tempList = list;
                } else {
                    for (BrandData data : list) {
                        if (data.getBrandName().toLowerCase().contains(constraint.toString().toLowerCase())) {
                            tempList.add(data);
                        }
                    }
                }
            ...
            protected void publishResults(CharSequence constraint, FilterResults results) {
                if (results != null) {
                    filterList = (List<BrandData>) results.values;
